...
The supplied Q4_K_M checkpoint needs no companion projector or tokenizer files.
...
through any model access. The service handles tokenization, prompt rendering,
context checks, canvas construction, serialization and lifecycle safety for you.
...
overflow return validation errors. Images, conditional question dependencies,
sequential question chaining, additional denoising steps, and thought generation
are rejected explicitly.

The server caps request bodies at 1 MiB. `TS_JEV_MAX_CANVAS` defaults to 64
...
These are probabilities conditional on the listed answers. A `noul` value is
